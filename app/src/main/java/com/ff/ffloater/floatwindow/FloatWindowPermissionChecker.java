...
public class FloatWindowPermissionChecker {
    ...
    public static boolean checkFloatWindowPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return Settings.canDrawOverlays(MainApplication.Companion.getInstance());
    ...
    private static boolean checkOps() {
        try {
            Object object = MainApplication.Companion.getInstance().getSystemService(Context.APP_OPS_SERVICE);
            if (object == null) {
                return false;
            }
            ...
            arrayOfObject1[2] = MainApplication.Companion.getInstance().getPackageName();
